...
uses
  ...
  Menus,Forms, Dialogs,CompItems,dbtables,db, ComWriUtils,LibMessages;
...
Note: to make sure assosiated component's ability is correct,
      directly call checkEnabled or  CheckVisible.
}
  // %TCustomAbilityManager : 能力管理的基类
  TCustomAbilityManager = class(TCompCommonAttrs)
  private
    ...
    FAbilityProvider: TCustomAbilityProvider;
    procedure setEnabled(value : boolean);
    procedure SetVisible(value : boolean);
    procedure SetVisibleOnEnabled(value : boolean);
    procedure SetAbilityProvider(const Value: TCustomAbilityProvider);
    procedure LMComAttrsChanged(var message : TMessage);message LM_ComAttrsChanged;
  protected
    { Protected declarations }
    procedure InternalCheckEnabled; virtual;
    procedure InternalCheckVisible; virtual;
    ...
    property  AbilityProvider : TCustomAbilityProvider read FAbilityProvider write SetAbilityProvider;
  ...
  { %TAbilityManager : 最简单的能力管理基类
   通过向客户控件发送LM_ComAttrsChanged，通知客户控件Enable,Visible发生改变。
   客户控件必须响应LM_ComAttrsChanged，同时
  }
  TAbilityManager = class(TCustomAbilityManager)
  published
    property  AbilityProvider;
    property  Enabled;
    property  Visible;
    property  VisibleOnEnabled;
    property  OnEnableChanged;
...
{TCustomAuthorityProvider : 权限提供者
 property:
...
//    procedure SetAbilityProviders(index : integer;value : TCustomAbilityProvider);
  ...
  TCustomAbilityProvider = class(TCompCommonAttrs)
  private
...
procedure TCustomAbilityManager.InternalCheckEnabled;
begin
  //UpdateClients;
  PropChanged;
end;

procedure TCustomAbilityManager.InternalCheckVisible;
begin
  //UpdateClients;
  PropChanged;
end;

procedure TCustomAbilityManager.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  if (AComponent=FAbilityProvider) and (Operation=opRemove) then
    FAbilityProvider:=nil;
  inherited Notification(AComponent,Operation);
end;

procedure TCustomAbilityManager.SetAbilityProvider(
  const Value: TCustomAbilityProvider);
begin
  if SetCommonAttrsProp(self,TCompCommonAttrs(FAbilityProvider),value) then
    UpdateEnabled;
end;

procedure TCustomAbilityManager.UpdateEnabled;
begin
  if FAbilityProvider<>nil then
    Enabled := FAbilityProvider.Enabled;
end;

procedure TCustomAbilityManager.LMComAttrsChanged(var message: TMessage);
begin
  if Message.WParam=WParam(FAbilityProvider) then
    UpdateEnabled;
end;
...
begin
  inherited InternalCheckEnabled;
  if FMenuItem<>nil
...
begin
  inherited InternalCheckVisible;
  if FMenuItem<>nil
...
begin
  ...
  if FAbilityProviders<>nil then
    FAbilityProviders.remove(ap);
end;
...
begin
  if FAbilityProviders=nil then exit;
