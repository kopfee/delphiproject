...
{**********************************************
   ...
   <History>
   <Guide>
   一、工作视图是用来描述数据输入/输出界面程序的模型。
   在这类程序(数据输入/输出)中：
   1、用户输入一些数据项目。
   2、在输入的过程中，根据当前输入的内容和服务器的数据，显示(输出)一些反馈信息。
   3、数据输入完毕以后提交到服务器处理，将处理结果(成功信息、流水号、查询结果集等等)输出给用户。
   整个过程中核心的是数据，所有的处理都应该围绕数据而展开。

   二、一个"工作字段"代表了一个数据项目。
   工作字段(TWVField)包含的语意有：
   1、名称(Name具有唯一性，用来标志一个字段)和标题(Caption显示的时候使用)。
   2、数据的数据类型(DataType)：整数、浮点数、对象(结果集以及其他)等等。
   3、数据的当前值(补充在Data属性里面，是TKSDataObject对象)和缺省值(DefaultValue)。
   4、数据的约束(有效性控制)(Constrained、Valid、Checker、DomainName等属性)
   5、文字性的补充描述(Description、Notes)
   6、处理基本的键盘输入，允许Enter(GoNextWhenPressEnter)切换到下一个输入控件、"*"或者其他特殊按键(GetPrevChar)切换到上一个输入控件
   7、将几个相关的工作字段编组(GroupIndex)。
   8、和数据源的字段(TField)绑定关系(DataField)。
   9、提供输入提示(Hint和ErrorMessage)。
   10、Script和Features提供了额外的扩充功能。
   11、在值或者有效性发生的时候触发事件。
   工作字段一般和界面上面的控件相关联。
   1、在界面输入的数据会自动保存到工作字段的当前值里面。
   2、当工作字段的当前值发生改变的时候，控件显示的数据也发生相应的改变。

   三、一个工作视图(TWorkView)包含了一个操作界面需要的所有数据项目对应的工作字段。
   工作视图提供对这些工作字段的统一管理。
   工作视图包含的语意有：
   1、提供对工作字段的访问：(WorkFields、FieldCount、Fields、FieldByName、FindField)
   2、将工作字段和输入/输出控件关联起来。
   3、提供统一的属性，例如(ValidColor、InvalidColor、ReadOnlyColor)
   4、统一处理意外等。
   5、管理监控器
   6、其他扩充机制，例如(Description、Notes、WorkProcesses、Users、Security、Entries、Exits)。

   四、监控器(TWVFieldMonitor)提供一种手段，可以监控多个字段的变化。
   当监控的字段的值或者有效性发生改变的时候可以触发特定的事件处理。

   五、字段检查器(TWVFieldChecker)提供一种对工作字段的值的有效性进行检查的机制。是工作字段的辅助类。
   字段检查器可以根据包含的字符、字符串长度、数值的大小进行校验。
   还可以根据触发事件进行特殊的判断。

   六、字段域(TWVFieldDomain)的目的是对一组同类的字段的值提供统一的检验方式。
   字段域在检查字段有效性的时候利用了字段检查器。

   七、工作视图中心对象(TWorkViewCenter)是管理应用程序中所有工作视图的管理对象。
    1、统一控制外观ValidColor、InvalidColor、ReadOnlyColor
    2、统一处理意外和输入错误。

   八、字段域(TWVFieldDomainManager)管理所有的字段域
...
触发处理的条件：
a)当字段为nil并且当前reset的AGroupIndex等于0或者和Monitor的GroupIndex相同；
b)字段<>nil,字段名称被包括在MonitorValueChangedFields，
}

{
键盘事件的处理过程，遵从从个别到一般的顺序(以KeyPress为例)：
1)控件的OnKeyPress，
2)字段的Checker的OnKeyPress，
3)字段的Checker的过滤
4)字段域的Checker的OnKeyPress，
5)字段域的Checker的过滤
目的是允许在Checker的过滤以前获得准确的键盘按键
}
{$I KSConditions.INC }
...
type
...

  {
    ...
    <Properties>
      ReserveNGroups - 是否在Reset的时候保护GroupIndex<0的字段。
      如果ReserveNGroups＝True，在Reset(0)的时候不复位GroupIndex<0的字段。
      否则(缺省情况下)，在Reset(0)的时候复位所有的字段。
    <Methods>
  ...
  public
    ...
    procedure   SelectPrev(Current : TWinControl);
    procedure   SelectNext(Current : TWinControl);
  ...
  published
    ...
    property    ReserveNGroups : Boolean read FReserveNGroups write FReserveNGroups default False;
  ...
  private
    ...
    procedure   DataObjectError(Sender : TKSDataObject; var E : Exception);
  ...
  public
    ...
    // Enter是否自动到下一个控件。
    // 如果设置了GoNextWhenPressEnter，那么每次按下Enter的时候，AutoGoNext:=True
    // 然后在字段的事件里面可以设置AutoGoNext:=False，暂停这次的自动下移。
...
var
  i : integer;
  ResetIt : Boolean;
  TheGroupIndex : Integer;
begin
  try
    FReseting := True;
    for i:=0 to FWorkfields.Count-1 do
    begin
      TheGroupIndex := TWVField(FWorkFields.Items[i]).GroupIndex;
      if AGroupIndex=TheGroupIndex then
        ResetIt := True
      else if AGroupIndex=0 then
      begin
        if TheGroupIndex>0 then
          ResetIt := True
        else // TheGroupIndex<0
          ResetIt := not ReserveNGroups;
      end
      else
        ResetIt := False;
      if ResetIt then
...
begin
  if Assigned(OnInvalidInput) then
...
procedure TWorkView.SelectNext(Current : TWinControl);
begin
  if Container<>nil then
    TWinControlAccess(Container).SelectNext(Current,True,True);
end;

procedure TWorkView.SelectPrev(Current : TWinControl);
begin
  if Container<>nil then
    TWinControlAccess(Container).SelectNext(Current,False,True);
end;
...
begin
  ...
  FData.OnException := DataObjectError;
  FDefaultValue := '';
  FConstrained := false;
  FDomainName := '';
  FDomain := nil;
  ...
  begin
    try
      ...
      begin
        ...
        // 然后用事件检查
        if Assigned(Checker.FOnCheckValid) then
...
begin
  ...
  *)
  ...
  if IsMonitoredField and Assigned(FOnUpdateValue) then
  begin
    // 利用FProcessingFieldChanged标志，如果在OnUpdateValue里面修改字段的值，不再触发事件，优化处理
...
begin
  ...
  // 使用保留的SavedKey，判断是否是'*'和'Enter'。
  if (GetPrevChar<>#0) and (SavedKey=GetPrevChar) and (WorkView.Container<>nil) and (Sender is TWinControl) then
  begin
    Key := #0;
    //TWinControlAccess(WorkView.Container).SelectNext(TWinControl(Sender),False,True);
    WorkView.SelectPrev(TWinControl(Sender));
  end;
  if (SavedKey=#13) and GoNextWhenPressEnter and (WorkView.Container<>nil) and (Sender is TWinControl) then
  begin
    FAutoGoNext := True;
    // 先同步数据
    ...
    begin
      ...
      if FAutoGoNext then
        WorkView.SelectNext(TWinControl(Sender));
    end else
    begin
      InvalidInput(Sender);
    end;
...
begin
  ...
  begin
    FValidChanged := False;
    if Assigned(FOnValidChanged) then
...
procedure TWVField.DataObjectError(Sender: TKSDataObject;
  var E: Exception);
begin
  E.Message := Format('[%s]%s',[Name,E.Message]);
end;
...
begin
  ...
  else if (Field.DataType in [kdtInteger,kdtInt64,kdtCurrency,kdtFloat]) and
    ((Min<>0) or (Abs(Max-Min)>0.0000001)) then
  begin
...
begin
  if SameText(ADataPresentType,Self.DataPresentType) and Assigned(FOnCtrlToField) then
...
procedure TWVFieldPresent.FieldToCtrl(Field: TWVField; Ctrl: TObject;
  const ADataPresentType: TDataPresentType; const Param : string; var Handled: Boolean);
begin
  if SameText(ADataPresentType,Self.DataPresentType) and Assigned(FOnFieldToCtrl) then
...
procedure TWVFieldPresent.SetFieldEventHanlder(Field: TField;
  const ADataPresentType: TDataPresentType; const Param : string; var Handled: Boolean);
begin
  if SameText(ADataPresentType,Self.DataPresentType) and Assigned(FOnSetFieldHandler) then
