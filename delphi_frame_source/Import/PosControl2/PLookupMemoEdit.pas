...
constructor TPLookupMemoEdit.Create(AOwner:TComponent);
begin
...
destructor TPLookupMemoEdit.Destroy;
begin
     FQuery.Free;
     inherited Destroy;
end;

function TPLookupMemoEdit.CheckSame: Boolean;
var str: string;
begin
...
function TPLookupMemoEdit.CheckExist: Boolean;
var str: string;
begin
...
procedure TPLookupMemoEdit.SetFullLength(Value: integer);
begin
...
procedure TPLookupMemoEdit.BackText;
begin
     if FBackText <> Text then
        FBackText := Text;
end;

procedure TPLookupMemoEdit.DoExtend;
var i:integer;
...
procedure TPLookupMemoEdit.Change;
begin
     if not FExtending then
        inherited Change;
end;

procedure TPLookupMemoEdit.KeyPress(var Key: Char);
begin
...
procedure TPLookupMemoEdit.DoExit;
begin
     if self.ClassType = TPLookupMemoEdit then
        BackText
     else
         text := FBackText;
     if  (Text <> '' )and (FEnPreFix <> False)then
...
procedure TPLookupMemoEdit.SetDataBaseName(Value: string);
begin
      FDatabaseName := Value;
      if Value = '' then
      begin
...
procedure TPLookupMemoEdit.SetForeignFieldName(Value: string);
begin
     FForeignFieldName := Value;
end;

procedure TPLookupMemoEdit.SetFieldName(Value: string);
begin
     FFieldName := Value;
end;

procedure TPLookupMemoEdit.SetTableName(Value: string);
begin
     FTableName := Value;
end;

procedure TPLookupMemoEdit.SetForeignTableName(Value: string);
begin
...
procedure Register;
begin
     RegisterComponents('PosControl2', [TPLookupMemoEdit]);
end;
